    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.current_joint_pos is None:
            ...
            self.current_joint_pos = np.array(
                [present[name] for name in self.bus.motors]
            )
        ...
        grip_delta[:3, 3] = np.array(
            [-action["delta_z"] * sx, action["delta_y"] * sy, action["delta_x"] * sz],
            dtype=np.float32,
        )
        grip_delta[:3, :3] = (
            _rot_x(delta_yaw) @ _rot_y(delta_pitch) @ _rot_z(delta_roll)
        )
